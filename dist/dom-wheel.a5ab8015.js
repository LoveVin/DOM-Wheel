...
window.dom = {
  ...
  //改
  text: function text(node, textString) {
  ...
  html: function html(node, htmlString) {
    if (arguments.length === 2) {
  ...
  index: function index(node) {
    var nodeList = node.parentNode.children;
  ...
  siblings: function siblings(node) {
    return Array.from(node.parentNode.children).filter(function (el) {
      return el !== node;
    });
  },
  ...
  //事件处理
  on: function on(node, eventType, fn) {
    node.addEventListener(eventType, fn);
  },
  off: function off(node, eventType, fn) {
    node.removeEventListener(eventType, fn);
  },
  delegate: function delegate(agentNode, eventType, clientSelector, fn) {
    agentNode.addEventListener(eventType, function (e) {
      var el = e.target;
